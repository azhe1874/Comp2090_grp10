...
class WarehouseApp:
    ...
    def __init__(self):
        ...
        # Explicitly declare global trees to avoid Notebook reference errors
    ...
    def show_main_menu(self):
        ...
        notebook.add(self.dash_frame, text="Dashboard")
        self.build_dashboard(self.dash_frame)
    ...
    def build_dashboard(self, parent):
        ...
        ttk.Button(btn_frame, text="Refresh Dashboard", command=self.refresh_dashboard).pack(side=RIGHT)
    ...
    def build_inventory_tab(self, parent, is_admin):
        ...
        ttk.Button(btn_frame, text="Refresh", command=lambda: self.refresh_inventory_tree(self.inventory_tree)).pack(side=LEFT, padx=10)
        
        ttk.Button(btn_frame, text="View Details", command=self.view_item_details).pack(side=LEFT, padx=10)
        ...
        self.refresh_inventory_tree(self.inventory_tree)
    ...
    def view_item_details(self):
        ...
        if not selected:
            messagebox.showwarning("Select Item", "Please select an item to view details.")
            return
            
        item_values = tree.item(selected[0])['values']
        item_id = str(item_values[0])
        item_name = str(item_values[1])

        # 获取历史记录和供应商信息
        data = self.controller.get_item_details_extra(item_id)
        
        dialog = Toplevel(self.root)
        dialog.title(f"Item Details - {item_name}")
        ...
        ttk.Label(info_frame, text=f"商品名称: {item_name}  |  商品ID: {item_id}", font=("Arial", 12, "bold")).pack(anchor=W)
        ttk.Label(info_frame, text=f"入库供应商: {data['suppliers']}", foreground="blue").pack(anchor=W, pady=(5,0))
        
        hist_frame = ttk.LabelFrame(dialog, text="商品历史记录 (Transaction History)", padding=5)
        hist_frame.pack(fill=BOTH, expand=True, padx=10, pady=10)
        
        cols = ("Trans ID", "Type", "Qty", "Operator", "Date", "Supplier")
        htree = ttk.Treeview(hist_frame, columns=cols, show="headings", height=10)
        
        htree.heading("Trans ID", text="交易编号")
        htree.heading("Type", text="进出库类型")
        htree.heading("Qty", text="数量")
        htree.heading("Operator", text="操作员")
        htree.heading("Date", text="时间")
        htree.heading("Supplier", text="涉及供应商")
        
        htree.column("Trans ID", width=120)
        htree.column("Type", width=80)
        htree.column("Qty", width=80)
        htree.column("Operator", width=100)
        htree.column("Date", width=150)
        htree.column("Supplier", width=120)
        
        scrollbar = ttk.Scrollbar(hist_frame, orient=VERTICAL, command=htree.yview)
        htree.configure(yscrollcommand=scrollbar.set)
        htree.pack(side=LEFT, fill=BOTH, expand=True)
        scrollbar.pack(side=RIGHT, fill=Y)
        
        for r in data['history']:
            htree.insert("", END, values=r)
    ...
    def build_suppliers_tab(self, parent, is_admin):
        ...
        ttk.Button(btn_frame, text="Refresh", command=lambda: self.refresh_supplier_tree(self.supplier_tree)).pack(side=LEFT, padx=10)

        ttk.Button(btn_frame, text="View Details", command=self.view_supplier_details).pack(side=LEFT, padx=10)
        ...
        self.supplier_tree = tree
        self.refresh_supplier_tree(self.supplier_tree)
    ...
    def view_supplier_details(self):
        tree = self.supplier_tree
        selected = tree.selection()
        if not selected:
            messagebox.showwarning("Select Supplier", "Please select a supplier to view details.")
            return
            
        sup_values = tree.item(selected[0])['values']
        sup_id = str(sup_values[0])
        sup_name = str(sup_values[1])
        
        items_supplied = self.controller.get_supplier_supplied_items(sup_id)
        
        dialog = Toplevel(self.root)
        dialog.title(f"Supplier Details - {sup_name}")
        ...
        ttk.Label(info_frame, text=f"供应商: {sup_name} ({sup_id})", font=("Arial", 12, "bold")).pack(anchor=W)
        
        items_frame = ttk.LabelFrame(dialog, text="累计供货明细", padding=5)
        items_frame.pack(fill=BOTH, expand=True, padx=10, pady=10)
        
        cols = ("Item ID", "Item Name", "Total Quantity Supplied")
        itree = ttk.Treeview(items_frame, columns=cols, show="headings")
        
        itree.heading("Item ID", text="商品编号")
        itree.heading("Item Name", text="商品名称")
        itree.heading("Total Quantity Supplied", text="历史供货总数")
        
        itree.column("Item ID", width=100)
        itree.column("Item Name", width=150)
        itree.column("Total Quantity Supplied", width=120)
            
        scrollbar = ttk.Scrollbar(items_frame, orient=VERTICAL, command=itree.yview)
        itree.configure(yscrollcommand=scrollbar.set)
        itree.pack(side=LEFT, fill=BOTH, expand=True)
        scrollbar.pack(side=RIGHT, fill=Y)
        
        for r in items_supplied:
            itree.insert("", END, values=r)
